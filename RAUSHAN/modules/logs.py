...
from config import X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, SUDO_USERS, OWNER_IDS, HEROKU_API_KEY, HEROKU_APP_NAME, CMD_HNDLR as hl
...
async def logs(legend):
    if legend.sender_id in OWNER_IDS:
        ...
        with open("AltLogs.txt", "w") as logfile:
            logfile.write("⚡ 𝐌 ᴀ ᴅ ᴀ ʀ ᴀ ⚡ [ Bot Logs ]\n\n" + logs)
        ...
        try:
            await X1.send_file(legend.chat_id, "AltLogs.txt", caption=f"⚡ ** 𝐌 ᴀ ᴅ ᴀ ʀ ᴀ  ʟᴏɢs*** ⚡\n  » **ᴛɪᴍᴇ ᴛᴀᴋᴇɴ:** `{ms} ꜱᴇᴄᴏɴᴅꜱ`")
            await fetch.delete()
